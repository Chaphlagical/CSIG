...
{
	const vec4 gbufferA_data = texelFetch(prev_gbufferA, ivec2(frag_coord), mip);
	const vec4 gbufferB_data = texelFetch(prev_gbufferB, ivec2(frag_coord), mip);
	const vec4 gbufferC_data = texelFetch(prev_gbufferC, ivec2(frag_coord), mip);
	const float depth = texelFetch(prev_depth_buffer, ivec2(frag_coord), mip).r;
	const ivec2 image_size = textureSize(prev_depth_buffer, mip);

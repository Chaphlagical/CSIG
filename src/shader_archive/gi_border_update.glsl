...
{
    ivec2 src_coord = current_coord + offsets[index].xy;
    ivec2 dst_coord = current_coord + offsets[index].zw;
#ifdef UPDATE_DEPTH
    imageStore(output_depth, dst_coord, imageLoad(output_depth, src_coord));
#else
    imageStore(output_irradiance, dst_coord, imageLoad(output_irradiance, src_coord));
#endif
}

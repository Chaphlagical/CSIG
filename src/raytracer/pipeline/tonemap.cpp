...
void Tonemap::draw(CommandBufferRecorder &recorder, const TAA &taa)
{
	recorder
	    .begin_marker("Tonemapping")
	    .insert_barrier()
	    ...
	    .insert()
	    .bind_pipeline(VK_PIPELINE_BIND_POINT_COMPUTE, m_pipeline)
	    .bind_descriptor_set(VK_PIPELINE_BIND_POINT_COMPUTE, m_pipeline_layout, {taa.descriptor.sets[m_context->ping_pong], m_descriptor.output_set})
	    .push_constants(m_pipeline_layout, VK_SHADER_STAGE_COMPUTE_BIT, m_push_constant)
	    .dispatch({m_context->render_extent.width, m_context->render_extent.height, 1}, {NUM_THREADS_X, NUM_THREADS_Y, 1})
	    .insert_barrier()
	    .add_image_barrier(render_target.vk_image,
...
void Tonemap::draw(CommandBufferRecorder &recorder, const Bloom &bloom)
{
	recorder
	    .begin_marker("Tonemapping")
	    .insert_barrier()
	    ...
	    .insert()
	    .bind_pipeline(VK_PIPELINE_BIND_POINT_COMPUTE, m_pipeline)
	    .bind_descriptor_set(VK_PIPELINE_BIND_POINT_COMPUTE, m_pipeline_layout, {bloom.descriptor.set, m_descriptor.output_set})
	    .push_constants(m_pipeline_layout, VK_SHADER_STAGE_COMPUTE_BIT, m_push_constant)
	    .dispatch({m_context->render_extent.width, m_context->render_extent.height, 1}, {NUM_THREADS_X, NUM_THREADS_Y, 1})
	    .insert_barrier()
	    .add_image_barrier(render_target.vk_image,
...
void Tonemap::update_descriptor()
{
	...
	m_context->update_descriptor()
	    .write_sampled_images(0, {render_target_view})
	    .update(descriptor.set);
}

void Tonemap::destroy_resource()
{
	m_context->destroy(render_target)
	    .destroy(render_target_view);
}

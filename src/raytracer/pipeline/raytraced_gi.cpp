...
void RayTracedGI::visualize_probe(VkCommandBuffer cmd_buffer, VkImageView color_image, VkImageView depth_image, const Scene& scene, const GBufferPass& gbuffer_pass)
{
	/*m_context->begin_marker(cmd_buffer, "DDGI Probe Visualization");
	{
		...
		VkDescriptorSet descriptors[] = {
			scene.descriptor.set,
		    gbuffer_pass.descriptor.sets[m_context->ping_pong],
		    m_probe_visualize.descriptor_sets[m_context->ping_pong],
		};
		vkCmdBindDescriptorSets(cmd_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, m_probe_visualize.pipeline_layout, 0, 3, descriptors, 0, nullptr);

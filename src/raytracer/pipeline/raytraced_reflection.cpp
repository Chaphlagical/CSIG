    ...
    m_context(&context)
{
	...
	// Create raytrace image
	{
		...
		// image_create_info.extent = VkExtent3D{m_context->extent.width, m_context->extent.height, 1};
	...
	// Create reprojection prev image
	{
		VkImageCreateInfo image_create_info = {
		    .sType         = VK_STRUCTURE_TYPE_IMAGE_CREATE_INFO,
		    .imageType     = VK_IMAGE_TYPE_2D,
		...
		vmaCreateImage(context.vma_allocator, &image_create_info, &allocation_create_info, &reprojection_prev_image.vk_image, &reprojection_prev_image.vma_allocation, nullptr);
		VkImageViewCreateInfo view_create_info = {
		    .sType            = VK_STRUCTURE_TYPE_IMAGE_VIEW_CREATE_INFO,
		    .image            = reprojection_prev_image.vk_image,
		    .viewType         = VK_IMAGE_VIEW_TYPE_2D,
		    .format           = VK_FORMAT_R16G16B16A16_SFLOAT,
		    .components       = {VK_COMPONENT_SWIZZLE_IDENTITY, VK_COMPONENT_SWIZZLE_IDENTITY, VK_COMPONENT_SWIZZLE_IDENTITY, VK_COMPONENT_SWIZZLE_IDENTITY},
		    .subresourceRange = {
		...
		vkCreateImageView(context.vk_device, &view_create_info, nullptr, &reprojection_prev_view);
		m_context->set_object_name(VK_OBJECT_TYPE_IMAGE, (uint64_t) reprojection_prev_image.vk_image, "Reflection Reprojection Previous Image");
		m_context->set_object_name(VK_OBJECT_TYPE_IMAGE_VIEW, (uint64_t) reprojection_prev_view, "Reflection Reprojection Previous Image View");
	}
	...
	// Create reprojection pass
	{
		...
		// Create descriptor set layout
		{
			VkDescriptorSetLayoutBinding bindings[] = {
			    ...
			    // Denoise tile dispatch args buffer
			    {
			        .binding         = 6,
			        .descriptorType  = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER,
			        .descriptorCount = 1,
			    ...
			    // Copy tile dispatch args buffer
			    {
			        .binding         = 8,
			        .descriptorType  = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER,
			        .descriptorCount = 1,
	...
	// Create atrous pass
	{
		...
		// Allocate descriptor set
		{
			...
			vkAllocateDescriptorSets(m_context->vk_device, &allocate_info, m_denoise.a_trous.descriptor_sets);
		}
...
RayTracedReflection::~RayTracedReflection()
{
	...
	vkDestroyImageView(m_context->vk_device, reprojection_prev_view, nullptr);
	...
	vmaDestroyImage(m_context->vma_allocator, reprojection_prev_image.vk_image, reprojection_prev_image.vma_allocation);
	...
	vkFreeDescriptorSets(m_context->vk_device, m_context->vk_descriptor_pool, 2, m_denoise.a_trous.descriptor_sets);
...
void RayTracedReflection::init(VkCommandBuffer cmd_buffer)
{
	...
	VkImageMemoryBarrier image_barriers[] = {
	    ...
	    },
	    {
	        ...
	        .srcQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED,
	        .dstQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED,
	        .image               = a_trous_image[m_context->ping_pong].vk_image,
	        .subresourceRange    = VkImageSubresourceRange{
	               .aspectMask     = VK_IMAGE_ASPECT_COLOR_BIT,
	               .baseMipLevel   = 0,
	               .levelCount     = 1,
	...
	vkCmdPipelineBarrier(
	    ...
	    VK_PIPELINE_STAGE_ALL_COMMANDS_BIT,
	    0, 0, nullptr, 4, buffer_barriers, 7, image_barriers);
}
...
void RayTracedReflection::update(const Scene &scene, const GBufferPass &gbuffer_pass, const BlueNoise &blue_noise, const LUT &lut)
{
	...
	VkDescriptorBufferInfo denoise_tile_data_buffer_info = {
	    .buffer = denoise_tile_data_buffer.vk_buffer,
	    .offset = 0,
	    .range  = VK_WHOLE_SIZE,
	};
	...
	VkDescriptorBufferInfo copy_tile_data_buffer_info = {
	    .buffer = copy_tile_data_buffer.vk_buffer,
	    .offset = 0,
	    .range  = VK_WHOLE_SIZE,
	};

	VkDescriptorBufferInfo copy_tile_dispatch_args_buffer_info = {
	    .buffer = copy_tile_dispatch_args_buffer.vk_buffer,
	    .offset = 0,
	    .range  = VK_WHOLE_SIZE,
	};
	...
	for (uint32_t i = 0; i < 2; i++)
	{
		VkWriteDescriptorSet writes[] = {
		    ...
		    },
		    {
		        ...
		        .descriptorType   = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER,
		        .pImageInfo       = nullptr,
		        .pBufferInfo      = &denoise_tile_data_buffer_info,
		        .pTexelBufferView = nullptr,
		    },
		    {
		        .sType            = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET,
		    ...
		    },
		    {
		        ...
		        .descriptorType   = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER,
		        .pImageInfo       = nullptr,
		        .pBufferInfo      = &copy_tile_data_buffer_info,
		        .pTexelBufferView = nullptr,
		    },
		    {
		        .sType            = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET,
		        ...
		        .descriptorType   = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER,
		        .pImageInfo       = nullptr,
		        .pBufferInfo      = &copy_tile_dispatch_args_buffer_info,
		        .pTexelBufferView = nullptr,
		    },
		};
		vkUpdateDescriptorSets(m_context->vk_device, 9, writes, 0, nullptr);
	}
...
void RayTracedReflection::draw(VkCommandBuffer cmd_buffer, const Scene &scene, const GBufferPass &gbuffer_pass, const BlueNoise &blue_noise, const LUT &lut)
{
	m_context->begin_marker(cmd_buffer, "Raytraced Reflection");
	{
		...
		m_context->begin_marker(cmd_buffer, "Reflection - Denoise");
		{
			...
			{
				m_context->begin_marker(cmd_buffer, (std::string("Reflection - Copy Tiles - ") + std::to_string(i)).c_str());
				{
					VkDescriptorSet descriptors[] = {

    ...
    m_context(&context)
{
	...
	m_temporal_accumulation.pipeline        = m_context->create_compute_pipeline((uint32_t *) g_ao_temporal_accumulation_comp_spv_data, sizeof(g_ao_temporal_accumulation_comp_spv_data), m_temporal_accumulation.pipeline_layout);
	...
	{
		m_context->update_descriptor()
		    .write_combine_sampled_images(0, scene.nearest_sampler, {raytraced_image_view})
		    .write_storage_images(1, {ao_image_view[i]})
		    .write_storage_images(2, {history_length_image_view[i]})
		    .write_combine_sampled_images(3, scene.nearest_sampler, {ao_image_view[!i]})
		    .write_combine_sampled_images(4, scene.nearest_sampler, {history_length_image_view[!i]})

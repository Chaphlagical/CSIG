...
vec2 texture_coord_from_direction(vec3 dir, int probe_index, uint width, uint height, uint probe_side_length) 
{
    ...
    vec2 probe_top_left_position = vec2(float(mod(float(probe_index), float(probes_per_row))) * probe_with_border_side,
...
vec3 sample_irradiance(vec3 P, vec3 N, vec3 V, DDGIUniform ddgi_buffer, sampler2D probe_depth, sampler2D probe_irradiance)
{
	ivec3 base_grid_coord = base_grid_coord(P, ddgi_buffer);
    vec3 base_probe_pos = grid_coord_to_position(base_grid_coord, ddgi_buffer);
    ...
    for (int i = 0; i < 8; ++i) 
    {
        ...
        int p = grid_coord_to_probe_index(probe_grid_coord, ddgi_buffer);
        vec3 probe_pos = grid_coord_to_position(probe_grid_coord, ddgi_buffer);
        ...
        if (ddgi_buffer.visibility_test == 1)
        {
            ...
            vec2 temp = textureLod(probe_depth, tex_coord, 0.0).rg;
            float mean = temp.x;
